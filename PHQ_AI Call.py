...
결과는 Plotly를 통해 시각화되며, 'results/PHQ/AI Call' 디렉토리에 저장됩니다.
"""
...
def initialize_beta(df, X_user_columns, X_item_columns):
    ...
    def calculate_avg(df, var):
        avg_1 = df.loc[df[var] == 1, 'depress'].mean()
        avg_0 = df.loc[df[var] == 0, 'depress'].mean()
...
def prepare_simulation_data(df, X_user, X_item, beta, interaction_columns):
    ...
    X_user = df
    X_user_np = df.drop(columns=['depress']).values
...
def visualize_results(results_TS, X_simulation, X_cycle, max_arm, beta, min_reward, max_reward):
    """AI Call 데이터셋의 PHQ2 결과를 시각화합니다."""
    output_dir = 'results/PHQ/AI Call'
    ...
    fig.update_layout(
        title='Cumulative Regrets of PHQ2 Scores for AI Call Dataset',
    ...
    fig_hist.update_layout(
        title='Distribution of PHQ2 Scores for AI Call Dataset (Histogram)',
    ...
    fig_box.update_layout(
        title='Distribution of PHQ2 Scores for AI Call Dataset (Box-Plot)',
